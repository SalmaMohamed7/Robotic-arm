define((require, exports, module) => {
  ...
  /* DAT GUI */
  const salma = gui.addFolder('salma geometry')
    ...
    anglesGui.add(anglesDeg, key).min(jointLimits[`J${i}`][0] * RAD_TO_DEG).max(jointLimits[`J${i++}`][1] * RAD_TO_DEG).step(1).listen().onChange(() => {
      ...
      for (const key in anglesDeg) {
        if (anglesDeg.hasOwnProperty(key)) {
          ...
          console.log(key+"sssssssssss")
          console.log(anglesDeg.hasOwnProperty(key)+"salmaaaaaaaa")
        }
      }
      // firebase.database().ref('angles').on('value',(data)=>{
      //      console.log(data.toJSON().A0)
      //     anglesRad['A0'] = data.toJSON().A0 * DEG_TO_RAD
      //     anglesRad['A1'] = data.toJSON().A1 * DEG_TO_RAD
      //     anglesRad['A2'] = data.toJSON().A2 * DEG_TO_RAD
      //     anglesRad['A3'] = data.toJSON().A3 * DEG_TO_RAD
      //     anglesRad['A4'] = data.toJSON().A4 * DEG_TO_RAD
      //     anglesRad['A5'] = data.toJSON().A5 * DEG_TO_RAD
      //  })
