...
define((require, exports, module) => {
  ...
  const logger = store => dispatch => (action, data) => {
    ...
    console.log('%cnew state: ', 'color:green', newState)
        if(action==='ROBOT_CHANGE_ANGLES' || action==='ROBOT_CHANGE_TARGET'){
          firebase.database().ref('angles').update({
            A0 : newState.angles.A0/ Math.PI * 180,
            A1 : newState.angles.A1/ Math.PI * 180,
            A2 : newState.angles.A2/ Math.PI * 180,
            A3 : newState.angles.A3/ Math.PI * 180,
            A4 : newState.angles.A4/ Math.PI * 180,
            A5 : newState.angles.A5/ Math.PI * 180
          })
        }
    console.groupEnd()
    return newState
  ...
  class Hmi {
    constructor() {
      ...
      const simulate = () =>(
        firebase.database().ref('angles').on('value',(data)=>{
              console.log(data.toJSON().A0)
              Robot.dispatch('ROBOT_CHANGE_ANGLES', {
                A0: data.toJSON().A0 * DEG_TO_RAD,
                A1: data.toJSON().A1 * DEG_TO_RAD,
                A2: data.toJSON().A2 * DEG_TO_RAD,
                A3: data.toJSON().A3 * DEG_TO_RAD,
                A4: data.toJSON().A4 * DEG_TO_RAD,
                A5: data.toJSON().A5 * DEG_TO_RAD,
              })
